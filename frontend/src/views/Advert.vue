<template>
  <div class=" h-screen">
    ...
    <main
      v-show="!loading"
      v-if="!isReportLayerShow"
      class=" h-5/6  xl:flex xl:justify-center xl:items-center"
    >
      <div class=" w-11/12 bg-white p-6 shadow-2xl block m-auto mt-10 xl:mt-auto xl:grid xl:grid-cols-9">
        <div
          v-if="advert.images"
          class=" h-72 xl:h-auto col-span-4 bg-cover bg-no-repeat bg-center flex flex-row justify-between items-center"
          :style="{ backgroundImage: 'url(' + advert.images[imageIndex] + ')' }"
        >
...
export default {
  ...
  async created(){
    ...
    .then(async res => {
      ...
      console.log(res)
      if(this.advert.userID === this.userCookie.id){
        return this.$router.push('/dashboard')
      }
      await axios
      .get(`${API_URL}/users/${res.data.userID}`)
    ...
    .then(res =>{
      ...
      const indexLikedAdvert = res.data.likedAdverts.findIndex(el => el === this.advert.id);
  ...
  methods: {
    ...
    async like(){
      ...
      .catch(err => console.log(err.status))
      this.likedAdverts.push(this.id)
    ...
    async unLike(){
      this.isLiked = false;
      const index = await this.likedAdverts.findIndex(el => el === this.id)
    ...
    increment(){
      if(this.imageIndex === this.advert.images.length - 1){
    ...
    decrement(){
      if(this.imageIndex === 0){
        return this.imageIndex = this.advert.images.length - 1;
      }
      this.imageIndex--;
    }
  }
