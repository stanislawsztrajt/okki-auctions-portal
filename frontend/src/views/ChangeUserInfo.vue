<template>
  ...
  />
  <Loading :isCenter="true" v-else-if="isLoading"/>
  <div v-else class="w-full h-screen">
    <div class="h-full">
      ...
      <div class="fixed w-full bottom-0 mb-4 z-50">
        <div
          v-if="validationError"
          class=" bg-red-100 text-red-900 rounded-b text-teal-900 px-4 py-3 shadow-md flex items-center w-3/4 md:w-1/3 m-auto"
          role="alert"
        >
          <div class="py-1"><svg class="fill-current h-6 w-6 text-teal-500 mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M2.93 17.07A10 10 0 1 1 17.07 2.93 10 10 0 0 1 2.93 17.07zm12.73-1.41A8 8 0 1 0 4.34 4.34a8 8 0 0 0 11.32 11.32zM9 11V9h2v6H9v-4zm0-6h2v2H9V5z"/></svg></div>
          <p class="font-bold">{{ validationText }}</p>
        </div>

        <div
          v-if="responseAlert"
          class="bg-green-100 mt-4 text-green-900 rounded-b text-teal-900 px-4 py-3 shadow-md flex items-center w-3/4 md:w-1/3 m-auto"
          role="alert"
        >
          <div class="py-1"><svg class="fill-current h-6 w-6 text-teal-500 mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M2.93 17.07A10 10 0 1 1 17.07 2.93 10 10 0 0 1 2.93 17.07zm12.73-1.41A8 8 0 1 0 4.34 4.34a8 8 0 0 0 11.32 11.32zM9 11V9h2v6H9v-4zm0-6h2v2H9V5z"/></svg></div>
          <p class="font-bold">{{ responseText }}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    async saveEditedUsername(){
      ...
      .then(async res => {
        ...
        this.responseText = `Pomyślnie zmieniono nazwę użytkownika`
    ...
    async saveEditedEmail(){
      ...
      .then(async res => {
        ...
        this.responseText = `Pomyślnie zmieniono adres email`
        return this.responseAlert = true
      })
      .catch(err => {
        this.isLoading = false
