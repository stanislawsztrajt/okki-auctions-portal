<template>
  <div id="SearchFilteringElements" class="grid gap-y-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
  ...
  methods: {
    filterAdverts(advertsCopy, appliedFilters) {  
      ...
      this.adverts = this.adverts.filter((advert) => {
        if(!advert.filters) return
        return this.appliedFilters.every(filter => advert.filters.includes(filter));
      })
